...
var tmpDir = require('os').tmpDir()
    , path = require('path')
    , idgen = require('idgen')
    , dataFile = path.join(tmpDir, 'node-relations-test-' + idgen() + '.json')

//console.log('dataFile', dataFile);

describe('save to datafile', function () {
  before(function () {
    relations.use(relations.stores.memory, {dataFile: dataFile});
...
describe('load from datafile', function () {
  before(function () {
    relations.use(relations.stores.memory, {dataFile: dataFile});
    relations.define('repos');
    relations.repos.addRole('owner', ['pull', 'push', 'administrate']);
